...
// Engineer: 
// 
// Create Date: 2021/09/27 00:12:04
// Design Name: 
// Module Name: reg_stack_phy
// Project Name: 
// Target Devices: 
// Tool Versions: 
// Description: 
...
module reg_stack_phy(
    input             clk,//时钟信号
    input             wen,//写使能端
    input      [4 :0] raddr1,//源操作数1地址
    input      [4 :0] raddr2,//源操作数2地址
    input      [4 :0] waddr,//目的操作数地址
    input      [31:0] wdata,//目的操作数数据
    output reg [31:0] rdata1,//源操作数1值
    output reg [31:0] rdata2,//源操作数2值
    
    //展示寄存器堆
    input      [4 :0] test_addr,
    output reg [31:0] test_data
    );
    //本部分给出的寄存器堆实验的参考设计方案中，共实现32个寄存器，其中0号寄存器读出的值恒为0，
    //寄存器堆为:异步读同步写，共有1个写端口和3个读端口。其中一个读端口为调试读端口,用于扫描读出32个寄存器，
    //给出了实验原理和设计框图
